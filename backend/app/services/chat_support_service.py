...
def _preferred_intent_aliases(preferred: str) -> set[str]:
    safe_preferred = _normalize_intent_key(preferred)
    if not safe_preferred:
        return set()

    aliases = {safe_preferred}
    family_aliases = {
        "definition": {"definition", "service_overview", "overview", "intro", "intro_definition"},
        "fit_use_case": {"fit_use_case", "use_case", "consult_case", "special_case", "fit_case"},
        "required_info": {
            "required_info",
            "required_document",
            "document_list",
        ...
        "pricing": {"pricing", "pricing_policy", "pricing_factor", "quote_input", "site_check", "price_quote", "quotation"},
        "limitations": {"limitations", "restricted_area", "condition", "warning", "constraint", "restriction"},
        "weight": {"weight", "weigh", "น้ำหนัก", "heavy", "weight_check", "weight_detail"},
        "booking_step": {"booking_step", "booking_flow", "booking_process", "book_step"},
        "booking_input": {"booking_input", "booking_info", "booking_detail", "book_input", "prepare_booking_info"},
        "booking_timing": {"booking_timing", "booking_time", "advance_booking", "lead_time", "booking_schedule"},
        "special_case": {"special_case", "custom_case", "edge_case", "booking_special_case"},
        "pricing_factor": {"pricing_factor", "price_factor", "rate_factor", "pricing_detail"},
        "quote_input": {"quote_input", "quotation_input", "quote_detail", "pricing_input", "price_input"},
        "pricing_policy": {"pricing_policy", "price_policy", "rate_policy", "pricing_rule", "minimum_charge_policy"},
        "site_check": {"site_check", "site_survey", "onsite_check", "site_assessment"},
        "claim_step": {"claim_step", "claim_flow", "claim_process", "claim_howto"},
        "claim_input": {"claim_input", "claim_detail", "claim_info", "incident_input"},
        "claim_evidence": {"claim_evidence", "evidence_requirement", "evidence_list", "claim_proof", "proof_required"},
        "claim_timeline": {"claim_timeline", "claim_eta", "claim_duration", "claim_time"},
        "nationwide": {"nationwide", "all_area", "all_regions", "service_area"},
        "upcountry": {"upcountry", "provincial_area", "interprovince", "ต่างจังหวัด"},
        "restricted_area": {"restricted_area", "restricted_zone", "hard_to_reach_area", "remote_area"},
        "check_area": {"check_area", "area_check", "coverage_check", "destination_check"},
        "document_list": {"document_list", "documents_list", "doc_list", "paper_list"},
        "required_document": {"required_document", "document_required", "mandatory_document", "document_requirement"},
        "pod": {"pod", "proof_of_delivery", "delivery_proof", "signed_pod"},
        "missing_document": {"missing_document", "document_missing", "missing_paper", "incomplete_document"},
        "transit_time": {"transit_time", "delivery_time", "eta", "sla_time"},
        "pickup_window": {"pickup_window", "pickup_time", "pickup_slot", "pickup_schedule"},
        "cutoff": {"cutoff", "cutoff_time", "last_cutoff", "round_cutoff"},
        "delay_factor": {"delay_factor", "delay_reason", "late_factor", "timeline_delay"},
        "service_overview": {"service_overview", "overview", "service_intro", "general_overview"},
        "handoff": {"handoff", "human_handoff", "agent_handoff", "contact_staff"},
        "consult_case": {"consult_case", "case_consult", "advice_case", "fit_case"},
    }
    aliases.update(family_aliases.get(safe_preferred, set()))
    return aliases


def _row_intent_matches_preferred(row_intent: str, preferred_aliases: set[str]) -> bool:
    safe_row_intent = _normalize_intent_key(row_intent)
    if not safe_row_intent or not preferred_aliases:
        return False

    if safe_row_intent in preferred_aliases:
        return True

    for preferred in preferred_aliases:
        if not preferred:
            continue
        if safe_row_intent.startswith(f"{preferred}_") or safe_row_intent.endswith(f"_{preferred}"):
            return True

    token_parts = {part for part in safe_row_intent.split("_") if part}
    return bool(token_parts & preferred_aliases)
...
def rows_for_preferred_answer_intent(intent: ChatIntent, rows: list[dict]) -> list[dict]:
    ...
    filtered = [row for row in rows if _row_intent_matches_preferred(row.get("intent") or "", preferred_aliases)]
    return filtered or rows
...
def direct_topic_intent_rows(intent: ChatIntent, user_message: str) -> list[dict]:
    ...
    preferred_aliases = _preferred_intent_aliases(preferred)
    candidate_rows = [row for row in candidate_rows if _row_intent_matches_preferred(row.get("intent") or "", preferred_aliases)]
    if not candidate_rows:
        return []
