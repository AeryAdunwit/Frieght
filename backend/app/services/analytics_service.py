...
class AnalyticsService:
    ...
    def get_visit_count(self) -> VisitMetricsResponse:
        page_views_total = get_total_visit_count()
    ...
    def register_visit(self, visitor_id: str) -> JSONResponse | VisitMetricsResponse:
        try:
            metrics = register_site_visit(visitor_id)
        except Exception:
    ...
    def update_chat_review(self, body: ChatReviewPayload) -> JSONResponse | ReviewUpdateResponse:
        ...
        note = sanitize_log_text(body.note, 500)
        owner_name = sanitize_log_text(body.owner_name, 120)
    ...
    def create_handoff_request(self, body: HandoffPayload) -> JSONResponse | dict[str, Any]:
        ...
        safe_session_id = sanitize_visitor_id(body.session_id) or "anonymous"
        safe_name = sanitize_log_text(body.customer_name, 120)
        safe_contact = sanitize_log_text(body.contact_value, 160)
        safe_channel = (body.preferred_channel or "phone").strip() or "phone"
        safe_note = sanitize_log_text(body.request_note, 800)
        safe_user_message = sanitize_log_text(body.user_message, 2000)
        safe_bot_reply = sanitize_log_text(body.bot_reply, 4000)
        safe_intent = sanitize_log_text(body.intent_name, 80)
        safe_source = sanitize_log_text(body.source, 80) or "chat_widget"
        safe_job_number = sanitize_log_text(body.job_number, 40)
    ...
    def update_handoff_request(self, body: HandoffUpdatePayload) -> JSONResponse | HandoffUpdateResponse:
        ...
        safe_note = sanitize_log_text(body.note, 800)
        safe_owner = sanitize_log_text(body.owner_name, 120)
    ...
    async def trigger_knowledge_sync(self) -> JSONResponse | SyncRunResponse:
        try:
            result = await execute_logged_sync("manual_admin", "admin_analytics")
        except Exception as exc:
    ...
    async def approve_to_sheet(self, body: SheetApprovalPayload) -> JSONResponse | dict[str, Any]:
        ...
        safe_question = sanitize_log_text(body.question, 500)
        safe_answer = sanitize_log_text(body.answer, 2000)
        safe_keywords = sanitize_log_text(body.keywords, 500)
        safe_intent = sanitize_log_text(body.intent, 120)
        safe_reason = sanitize_log_text(body.reason, 500)
        ...
        failed_rows = 0
        try:
            sync_result = await execute_logged_sync(
                "approve_to_sheet",
    ...
    def save_chat_feedback(self, body: ChatFeedbackPayload) -> JSONResponse | dict[str, Any]:
        ...
        try:
            ...
            matched_log = find_matching_chat_log_for_feedback(
                safe_session_id,
                safe_user_message,
                safe_bot_reply,
            )
