...
create table if not exists sheet_approvals (
  id bigint generated by default as identity primary key,
  chat_log_id bigint references chat_logs(id) on delete set null,
  topic text not null,
...
create index if not exists sheet_approvals_created_at_idx
  on sheet_approvals (created_at desc);

create index if not exists sheet_approvals_chat_log_id_idx
  on sheet_approvals (chat_log_id);
...
create index if not exists handoff_requests_owner_name_idx
  on handoff_requests (owner_name);

create index if not exists knowledge_sync_runs_created_at_idx
  on knowledge_sync_runs (created_at desc);
